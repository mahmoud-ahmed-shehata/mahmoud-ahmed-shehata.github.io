...
(function () {
  ...
  const navLinks = document.querySelectorAll('.nav-links a[data-section], .nav-drawer a[data-section]');
  if (navLinks.length) {
    const sections = [];
    navLinks.forEach(link => {
      ...
      if (id) {
        ...
        if (el) sections.push({ el, links: document.querySelectorAll(`[data-section="${id}"]`) });
      }
    });
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          sections.forEach(s => s.links.forEach(l => l.classList.remove('active')));
